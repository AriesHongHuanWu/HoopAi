...
async function migrate(db: SQLite.SQLiteDatabase): Promise<void> {
  ...
  if (version < 6) {
    // v6: split "the user corrected the OUTCOME" out of the overloaded
    // `corrected` flag. updateShotValue (the one-tap 2↔3 point fix) also
    // stamps corrected=1, so the hard-example export — which needs shots
    // whose MAKE/MISS call was wrong — was polluted by value-only fixes
    // masquerading as outcome corrections. outcomeCorrected is set only by
    // updateShotOutcome when the caller passes corrected=true (user edits;
    // machine rechecks/undo pass false and never stamp it).
    //
    // Backfill from the legacy flag: pre-v6 rows can't distinguish the two
    // kinds of correction, and copying keeps every previously-exported
    // outcome correction in the export instead of silently dropping them —
    // the same behavior those rows had before this migration.
    await db.execAsync(`
